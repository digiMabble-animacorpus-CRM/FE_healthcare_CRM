...
// Response interface for appointment distribution API
export interface AppointmentDistributionResponse {
  distribution: AppointmentDistributionItem[];
}
...
export const getAppointmentDistribution = async (
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error('Failed to fetch appointment distribution:', response.status, errorText);
      return null;
    }

    const result = await response.json();

    // The data structure is in result.data
    return {
